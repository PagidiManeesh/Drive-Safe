...
function onHeartRateDataChange(heartRateInfo) {
    ...
    if (rate < 1) {
        ...
        console.log("Hear Rate is ", rate + "  Time is :", datetime);
        //WriteToFile(rate, datetime);
        var obj = {"HeartRate": rate, "Time ": datetime};

        localStorage.setItem('D:\HeartRate.json', JSON.stringify(obj));
       /* var Object = {
                "HeartRate": rate,
                "DataStamp": datetime
            };

            fs.writeFile("./object.json", JSON.stringify(Object), (err) => {
                if (err) {
                    console.error(err);
                    return;
                };
